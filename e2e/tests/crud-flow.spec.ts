...
test("percorre login e CRUD completo, deixando todas as exclusões para o final", async ({ page }) => {
  ...
  const slotsHeading = scheduleDialog.getByRole("heading", { name: /-feira|sábado|domingo/i })
  await expect(slotsHeading).toBeFocused()
  await expect(scheduleDialog.getByRole("link", { name: "Voltar ao calendário" })).toHaveAttribute("href", /^#.+/)
