...
import type { ProfileFormState } from "../../features/dashboard/profileForm";
    ...
    async loadDashboard() {
      ...
      const [pets, customers, customerProfile] = await Promise.all([
        ...
        user.role === "cliente"
          ...
          : Promise.resolve<Customer | undefined>(undefined),
      ]);
      return {
        user,
        pets,
        ...
        profile:
          user.role === "profissional"
            ? findProfessionalProfile(user, professionals)
            : customerProfile,
      };
    },
    ...
    updateProfile(role: AuthUser["role"], payload: ProfileFormState) {
      if (role === "profissional") {
        return repository.updateProfessionalProfile({
          ...
          almoco_inicio: payload.almoco_inicio,
          almoco_fim: payload.almoco_fim,
        });
      }
